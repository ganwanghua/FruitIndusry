 ...
 */
public class NumberButton1 extends LinearLayout implements View.OnClickListener, TextWatcher {
    //库存
    ...
    public NumberButton1(Context context) {
        this(context, null);
    }

    public NumberButton1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
//
...
//    }
    ...
    private void init(Context context, AttributeSet attrs) {
        LayoutInflater.from(context).inflate(R.layout.layout_1, this);

        RelativeLayout addButton = (RelativeLayout) findViewById(R.id.rl_shop_insert);
        addButton.setOnClickListener(this);
        RelativeLayout subButton = (RelativeLayout) findViewById(R.id.rl_shop_delete);
        subButton.setOnClickListener(this);

        mCount = ((EditText) findViewById(R.id.et_shop_num));
    ...
    public void onClick(View v) {
        int id = v.getId();
        int count = getNumber();
        if (id == R.id.rl_shop_delete) {
            if (count > 1) {
                //正常减
                mCount.setText("" + (count - 1));
            }

        } else if (id == R.id.rl_shop_insert) {
    ...
    public NumberButton1 setCurrentNumber(int currentNumber) {
    ...
    public NumberButton1 setInventory(int inventory) {
    ...
    public NumberButton1 setBuyMax(int buyMax) {
        mBuyMax = buyMax;
        return this;
    }

    public NumberButton1 setOnWarnListener(OnWarnListener onWarnListener) {
